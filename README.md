...
| 最低 / 目标 SDK | `26 / 36` |
| ABI | `arm64-v8a` |
| 文件大小 | `39,239,855` 字节（`37.42 MiB`） |
| SHA-256 | `4D4497B89814BD6336C275029FC207846984192CD63AA7B7268010EEA12596C0` |
...
- 按 OPF spine 顺序解析 XHTML，并把正文与图片放进统一小说阅读流。
- 读取 EPUB 包内外链 CSS、文档内 `<style>` 和行内 `style`。
- 支持常用标签、类、ID、后代和直接子级选择器，以及对齐、相对字号、字重、斜体、下划线、行高、字距、首行缩进、上下边距、文字色、背景色和本地背景图。
- 支持 EPUB 包内 JPEG、PNG、GIF、WebP、BMP 及 base64 图片；不请求远程图片或远程 CSS。
- 同步保留纯文本正文，用于目录字数、文本选择、全文搜索和字符锚点跳转。
