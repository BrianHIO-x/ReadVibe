...
class StorageService {
  ...
  Future<File> _chapterBackupFile(String bookId) async {
    final file = await _chapterFile(bookId);
    return File('${file.path}.presplit');
  }

  /// Replaces a text book's chapters with a new split (smart AI re-split).
  /// The previous chapter list is kept in a sibling backup file so the change
  /// can be undone, and stored reading progress is remapped by overall
  /// reading position instead of being dropped.
  Future<void> resplitBookChapters(
    Book book,
    List<Chapter> newChapters,
  ) async {
    if (book.isPdf || book.chapters.isEmpty || newChapters.isEmpty) {
      throw const FormatException('章节数据无效');
    }
    final backup = await _chapterBackupFile(book.id);
    final payload = await Isolate.run(() => _chaptersToJson(book.chapters));
    await backup.writeAsString(payload, encoding: utf8, flush: true);
    ...
    final updated = Book(
      ...
      txtParserVersion: book.txtParserVersion,
    );
    await saveBook(updated);
    final progress = await getProgress(book.id);
    if (progress != null) {
      await saveProgress(
        _remapProgressAfterReparse(progress, book.chapters, newChapters),
      );
    }
  }

  Future<bool> hasChapterBackup(String bookId) async {
    try {
      return await (await _chapterBackupFile(bookId)).exists();
    } on Object {
      return false;
    }
  }

  /// Restores the chapter list saved before the last smart re-split. Returns
  /// false when no usable backup exists.
  Future<bool> restoreChapterBackup(Book book) async {
    if (book.isPdf) return false;
    try {
      final backup = await _chapterBackupFile(book.id);
      if (!await backup.exists()) return false;
      final raw = await backup.readAsString(encoding: utf8);
      final restored = await Isolate.run(() => _chaptersFromJson(raw));
      ...
      if (progress != null) {
        await saveProgress(
          _remapProgressAfterReparse(progress, book.chapters, restored),
        );
      }
      await backup.delete();
      return true;
